...
Sometimes we want to divide the plane into rectangular bins and compute a summary statistic within each bin.
The function `stat_summary_2d()` does exactly that.
Below, it computes the average judge score within bins of diver rank and dive difficulty.

```{r stat_summary_2d}
ggplot(diving, aes(x = Rank, y = Difficulty, z = JScore)) +
  stat_summary_2d() +
  labs(x = "Diver Rank", y = "Difficulty", fill = "Mean Score")
```

If your data are already summarized into a rectangular grid, `geom_tile()` is often the most direct way to build a heat map.
...
The `annotate()` function lets us add reference shapes or text to a plot.
For example, we can draw a rectangle to highlight a region of a scatterplot.

```{r annotate}
ggplot(diving_grouped, aes(x = Rank_mean, y = JScore_mean)) +
  geom_point() +
  annotate("rect", xmin = 1, xmax = 10, ymin = 7.5, ymax = 9.5,
           alpha = 0.1, color = "red")
```
...
This will load up a look-up table with all identifiers joining various sources together. It may take a couple minutes to run and may print warning messages. We find that Aaron Judge's unique `mlbam_id` is 592450. Using this id, we can grab all pitches thrown to Judge in the current MLB season so far:
...
Using this shot data, we can view all shot attempts by the Warriors throughout the season, colored by whether or not they made the shot.
...
It's apparent from this chart how the three-point line affects their shot selection.
