...
In the code above, we first overwrote `diving_hist` so that it is now a histogram of judges' scores where each bin has width 0.25. In the second line, we asked R to display this object. **For ggplot code, it is very important that the `+` goes at the end of a line, just like the pipe `%>%`.**
