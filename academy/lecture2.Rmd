...
We're going to pick up right where we left off yesterday in [Problem Set 1](ps1.html), where you wrote a script to create a tbl containing several shooting statistics for NBA players between the 1996-97 season and the 2015-16 season. Your script should look something like the code block below. For reasons that will become clear shortly, we are going to call our tbl `raw_shooting`. 
...
You can learn much, much more about ggplot2 [here](https://ggplot2.tidyverse.org). We will cover some of the more advanced functionality of ggplot2 in later lectures.
...
The first bit of the code above, `ggplot(data = raw_shooting)`, tells R to create a coordinate system to which we can add additional layers.
It also tells R that the data to be plotted is coming from the tibble `raw_shooting`.
...
It is often preferable to use a density histogram instead of a frequency histogram. This is because the areas of the density histogram boxes always sum to 1, no matter how much data you have. 
...
Try making histograms of `FTP`, `TPP`, and the other variables created above, and compare the shapes of the distributions.
...
Just like the code used to produce the histograms above, we began by calling `ggplot(data = raw_shooting)`. Now, however, instead of calling `geom_histogram()`, we are calling `geom_point()`. This tells R that instead of a histogram, we want to plot the data using points. We will learn much more about the different "geom" objects available in ggplot2 in [Lecture 5](lecture5.html). Since we are plotting multiple variables, we need to tell R to which axis each variable is mapped. This is exactly what `mapping = aes(x = FGP, y = FTP)` does. 
...
The code below filters out all of the players with at least 100 field goal attempts in a single season.
...
Sometimes a column that should be numeric gets read in as characters, especially when some values are missing.
We can convert a column to integers with `as.integer()` inside `mutate()`.
Missing values are recorded as `NA`, and we can replace them with a chosen value using `replace_na()`.
...
replace_na(example_stats, list(IBB = 0, HBP = 0))
```
