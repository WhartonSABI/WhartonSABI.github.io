...
ggplot(pbp_grouped, aes(x = attempts, y = avg_yards_gained)) +
  ...
  theme_minimal()
```
9. Now, let's add a third variable. Set the color to equal `avg_cpoe` and use `scale_color_viridis_c()` to set the color scale.
...
pbp_explosive_slice <- pbp_explosive %>%
  ...
  group_by(play_type) %>%
  arrange(desc(n_explosive), .by_group = TRUE) %>%
  slice_head(n = 10)
