...
3. Let's take a preliminary look at how location might affect pass outcomes. First, use `!is.na()` with `filter()` to filter out pass attempts without a location label. Then create a bar plot to visualize the frequency of attempts in each pass location.
...
4. Let's look at the distribution of yards gained based on pass location with some box plots. Use `geom_boxplot()` to create a box plot of `yards_gained` by `pass_location`.
```{r}
ggplot(pbp_pass, aes(x = pass_location, y = yards_gained)) +
  geom_boxplot()
```

5. Completion yards over expected is a metric that adjusts for the difficulty of a QB's throw by calculating the yards completed above the expected amount based on throw timing, coverage, and location. Take a look at how `cpoe` may differ based on the pass location alone with `geom_violin()`. What conclusions can you draw about pass location and pass outcomes?
```{r}
...
7. We can first visualize how attempts and avg_yards_gained are related by using a scatterplot.
...
ggplot(pbp_grouped, aes(x = attempts, y = avg_yards_gained)) +
  ...
  theme_minimal()
```
9. Now, let's add a third variable. Set the color to equal `avg_cpoe` and use `scale_color_distiller()` to set the color scale.
...
```
What do these graphs suggest about how quarterbacks differ in their average yards gained and average cpoe?
...
head(pbp_explosive)
```
12. Let's visualize the distribution of explosive plays by defensive team. Use `geom_bar()` to create a bar plot of the number of explosive plays by `defteam`.
```{r}
...
14. Finally, let's visualize the relationship between explosive plays and yards to go with a heat map. First, run the following code below to obtain the needed columns for this plot.
...
What conclusions can you draw from the visualizations in this section?
