...
It turns out that kickers make just over 83% of their attempts. However, we know that there are some truly elite kickers (e.g. Justin Tucker) who make well over 83% of their attempts, and other kickers (e.g. Billy Cundiff) who make less than 83% of their attempts. Instead of forecasting field goal success probabilities with the overall average, we could compute each individual kicker's conversion rate. This can be done using `group_by()` and `mutate()`. In the code below, we add a column to `fg_train` called "phat_kicker", which contains each kicker's individual field goal conversion rate. Notice that because we are adding a grouping to carry out this computation, we need to remove the grouping using `ungroup()` when we're done.
...
We also could have binned field goal distance into 5- or 2-yard increments, saving bin labels into columns called "Dist_5" or "Dist_2". Similar to the 10-yard bins, we could add columns to `fg_train` that compute the overall conversion rate within each of these 5-yard or 2-yard bins.
...
fg_plot  <- 
  ...
  geom_point(aes(x = Distance, y = phat_dist_5), alpha = 0.1, col = 'lawngreen') + 
  geom_point(aes(x = Distance, y = phat_dist_2), alpha = 0.1, col = 'white')
fg_plot 
```
Now when we bin our data into 2-yard increments, we find that our forecasts are no longer monotonic decreasing. The probability of making a field goal decreases overall, but in our 2-yard bins, the probability increases from 44 to 46 yards and from 52 to 54 yards. Remember, monotonic means only decreasing or only increasing for all values. In [Lecture 8](lecture8.html), we'll address this with a more formal regression method. 


## Assessing Predictions

We now have several predictive models of field goal success: phat_all, phat_kicker, phat_dist_10, phat_dist_5, and phat_dist_2. Recall from [Lecture 5](lecture5.html) that to assess how well we are predicting a continuous outcome, we could use the RMSE. When predicting a binary outcome, we have a few more options. To set the stage, let $y_{i}$ be the outcome of the $i^{\text{th}}$ observation and let $\hat{p}_{i}$ be the forecasted probability that $y_{i} = 1.$ (i.e. of making the FG)

The [Brier Score](https://en.wikipedia.org/wiki/Brier_score) is defined as
$$
BS = \frac{1}{n}\sum_{i = 1}^{n}{(y_{i} - \hat{p}_{i})^{2}}
$$
Looking at the formula, we see that the Brier score is just the mean square error of our forecasts. Using code that is very nearly identical to that used to compute RMSE in [Lecture 5](lecture5.html), we can compute the Brier score of each of our prediction models.
