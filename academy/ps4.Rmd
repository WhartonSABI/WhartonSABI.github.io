...
4. To estimate the probability of a called strike given the pitch location, we will divide the plane into several small rectangular bins and compute the proportion of called strikes within each bin. To compute this, we use the `stat_summary_2d()` function, which takes three aesthetics:
...
3. Use `filter()` to create a new tbl called `reduced_boxscore` that pulls out the rows of `team_boxscore` corresponding to the following teams: BOS, CLE, DAL, DET, GSW, LAL, MIA, and SAS. Then create a line plot of these teams' three point percentage in each season.
...
4. Use `pivot_longer()` on `reduced_boxscore` to create separate rows for each team's FGP, TPP, and FTP. Create a tibble filtered on one team only and visualize how their FGP, TPP, and FTP have all evolved over time on one plot.
```{r pt2 q4 solution}
...
ggplot(batting_tto, aes(x = ORDER_CT, y = EVENT_WOBA_19)) +
  ...
  theme_minimal()
```
3. Next, try summarizing your data with `reframe()` or reshaping it with `pivot_longer()` or `pivot_wider()` to generate another visualization. For example, plotting the mean event wOBA with `geom_col()`.
