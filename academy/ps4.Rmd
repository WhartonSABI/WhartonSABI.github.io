...
ggplot(batting_tto, aes(x = ORDER_CT, y = EVENT_WOBA_19)) +
  ...
  theme_minimal()
```
3. Next, try summarizing your data with `reframe()`, reshaping it with `pivot_longer()` or `pivot_wider()`, or computing correlation with `cor()` to generate another visualization. For example, plotting the mean event wOBA with `geom_col()`.
