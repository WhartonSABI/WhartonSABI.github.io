...
When using `replace_na()`, remember to include the `list(...)` argument that specifies the replacement value for each column. 

1. Load the Lahman data and run the above code to create the tbl `batting`, which has replaced all of the `NA` values in the columns `IBB`, `HBP`, `SH`, `SF`, and `GIDP`. 
...
3. Standardize each of BA, OBP, OPS, and wOBA using data from all of the years. Name the columns containing these new standardized values `zBA_all`, `zOBP_all`, etc. *Remember, you can do this with the `scale()` function from [Lecture 3](lecture3.html).* Who were the best and worst batters according to these four metrics?

```{r pt1 q3 solution}
# solution
batting <- batting %>%
  mutate(zBA_all = scale(BA),
         zOBP_all = scale(OBP),
         zOPS_all = scale(OPS),
         zwOBA_all = scale(wOBA))
...
batting <- batting %>%
  group_by(yearID) %>%
  mutate(zBA_year = scale(BA),
         zOBP_year = scale(OBP),
         zOPS_year = scale(OPS),
         zwOBA_year = scale(wOBA))
...
batting <- 
  ...
  mutate(zBA_year_lg = scale(BA),
         zOBP_year_lg = scale(OBP),
         zOPS_year_lg = scale(OPS),
         zwOBA_year_lg = scale(wOBA))
...
batting %>% 
  arrange(desc(zBA_year_lg)) %>%
  head(5)

batting %>% 
  arrange(zOBP_year_lg) %>%
  head(5)

batting %>% 
  arrange(desc(zOBP_year_lg)) %>%
  head(5)

batting %>% 
  arrange(zOPS_year_lg) %>%
  head(5)

batting %>% 
  arrange(desc(zOPS_year_lg)) %>%
  head(5)

batting %>% 
  arrange(zwOBA_year_lg) %>%
  head(5)

batting %>% 
  arrange(desc(zwOBA_year_lg)) %>%
  head(5)
```

6. Remove the grouping you created in Problem 5. Bill James [divided baseball history into several eras](https://www.billjamesonline.com/dividing_baseball_history_into_eras/) as follows:
...
batting <- batting %>%
  group_by(HIST_ERA) %>%
  mutate(zBA_hist = scale(BA),
         zOBP_hist = scale(OBP),
         zOPS_hist = scale(OPS),
         zwOBA_hist = scale(wOBA))
...
1. Read in the [MLB Payroll Data](data/mlb_payrolls.csv) and load it into a tibble called `mlb_payrolls`.
...
2. Using the pipe `%>%`, `group_by()`, and `mutate()`, add a column to `mlb_payrolls` that contains the relative payroll for each team. To do this, divide each team's payroll by the median payroll in that season.
...
mlb_payrolls <- 
  mlb_payrolls %>%
  group_by(Year) %>%
  mutate(Relative_Payroll = Team_Payroll / median(Team_Payroll))
```

3. Make a scatterplot of winning percentage against relative payrolls. Comment on the relationship. Your scatterplot should be identical to one you made in [Problem Set 2](ps2.html).
...
4. Using the `reframe()` function, compute the average team payroll and relative payroll for each year. Save these results in a new tbl called `payroll_avg`. 
...
5. Make a scatterplot that shows how team payrolls have evolved over the years. Similar to what we did in [Lecture 3](lecture3.html), add a line to this scatterplot that shows the average team payroll. Do the same thing for relative payroll. What do you notice about the average team payroll and relative payroll? 
...
6. Correlation is a measure of the strength of the *linear* relationship between two variables. The closer to +1 or -1 the correlation between two variables is, the more predictable they are of each other. We can compute it using the `cor()` function. Using `group_by()`, `reframe()`, and `cor()`, compute the correlation between relative payroll and winning percentage within each year. What do you notice about how the relationship between winning percentage and relative payroll changes year to year? 
...
Without running the code, see if you can figure out what the code below is doing.
...
Now, run the code above and save the tbl `batting_2014_2015` to the file `"data/batting_2014_2015.RData"` using the `save()` function. We will return to this dataset in [Lecture 4](lecture4.html).
