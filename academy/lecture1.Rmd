...
The *working directory* is the place where R saves output files and where it looks for data that you want to read in.
In [Lecture 0](lecture0.html), we asked you to create a folder on your Desktop called Moneyball and save <a href="Moneyball.Rproj" download="Moneyball.Rproj">Moneyball.Rproj</a> inside it.

From now on, the easiest way to start your work is to open `Moneyball.Rproj` in RStudio.
When you open that project, RStudio automatically uses the Moneyball folder as your working directory.
We can always check the current working directory with the code `getwd()`.

**Important: for the rest of the course, start by opening the Moneyball project rather than resetting the working directory by hand.**


## Base R
Now that we have opened our project, we can work with data. The basic functionality included with R is called Base R. Simple arithmetic such as `2*3` and `mean(3:9)` are included as initial functionality in base R. We can also assign values to vectors and plot graphs.
...
To access tidyverse, first we need to install the tidyverse package:
...
library(tidyverse)
```
When you do that, you'll see a lot of output to the console, most of which you can safely ignore for now. **Note that you only need to install the package once. However, each time you open up a new R session and want to use the tidyverse, you will need to run the line** `library(tidyverse)` **at the start.**
