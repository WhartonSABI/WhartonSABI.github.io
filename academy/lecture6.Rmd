...
Now let's go back to the 2014 and 2015 batting data. To visualize our correlation, we draw the *line of best fit* through our data. In this example, the line of best fit has $y$-intercept = 0.141 and slope = 0.485 (we'll talk about how we find these values later). 
...
We can plot the data by feeding these coefficients to `geom_abline()`, however this approach only works for linear regression. A more efficient way is to use `predict()` to add predicted values from the model to our dataset, sort the data by the predictor, and then add these points to our plot with `geom_line()`. We can do this as below:
...
batting_2014_2015 <- batting_2014_2015 %>% 
  add_predictions(model = model, type = "response", var = "pred") %>%
  add_residuals(model = model, var = "resid")
batting_2014_2015
```
...
We similarly have to specify a `model` and `var` argument in `add_residuals()`. 
